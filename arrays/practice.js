...
const divider = (nArr) => {
  return [nArr.filter(e => e % 2 === 0), nArr.filter(e => e % 2 !== 0)]
}
...
const removeItem = (myGroceryList, itemToRemove) => {
  ...
  if(myGroceryList) {
    for(let i = 0; i < myGroceryList.length; i++) {
      if(myGroceryList[i] === itemToRemove) myGroceryList.splice(i, 1)
    }
...
const addItem = (myGroceryList, itemToAdd) => {
  if(!itemToAdd)
    return []
  if(!myGroceryList) {
    return []
...
var user3 = {
  name: 'Sue',
  email: 'ILoveOldMen@outlook.com',
  password: 'ilove(oldmen)',
  username: 'jerryspringerslover'
}

var user4 = {
  name: 'Mary',
  email: 'iloveprotein@bit.coin',
  password: 'proteinislife4ever',
  username: 'dontmesswithmary1'
}
...
users.forEach((e) => {if(e.email === 'mark.mciver@devmounta.in') users.splice(users.indexOf(e), 1)})

// for( e in users) {
//   if(e.email === 'mark.mciver@devmounta.in') users.splice(users.indexOf(e), 1)
// }
